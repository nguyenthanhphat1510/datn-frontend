...
export default function ThuocBVTVPage() {
  /* ── Catalog state ── */
  const [thuocCategory, setThuocCategory] = useState<Category | null>(null);
  ...
  /* ── Nhóm thuốc = subcategory. "" = Tất cả (không lọc) ── */
  const [activeSubId, setActiveSubId] = useState("");
  /* ── Thương hiệu = nhà sản xuất. "" = Tất cả (không lọc) ── */
  const [activeManuId, setActiveManuId] = useState("");

  /* ── Tìm category "Thuốc BVTV" theo slug ── */
  useEffect(() => {
    fetchCategories()
      .then((cats) => {
        const found = cats.find((c) => c.slug === THUOC_BVTV_SLUG && c.isActive);
        setThuocCategory(found ?? null);
      })
      .catch(() => setThuocCategory(null))
      .finally(() => setCategoryLoaded(true));
  }, []);

  /* ── Nạp danh mục con của Thuốc BVTV để làm bộ lọc nhóm ── */
  useEffect(() => {
    if (!thuocCategory) {
      setSubcategories([]);
      return;
    }
    fetchSubcategories(thuocCategory._id)
      .then((subs) => setSubcategories(subs.filter((s) => s.isActive)))
      .catch(() => setSubcategories([]));
  }, [thuocCategory]);

  /* ── Nạp thương hiệu CÓ sản phẩm thuốc BVTV để làm bộ lọc ── */
  useEffect(() => {
    if (!thuocCategory) {
      setManufacturers([]);
      return;
    }
    fetchManufacturers(thuocCategory._id)
  ...
  }, [categoryLoaded, thuocCategory, page, search, priceRange, activeSubId, activeManuId, reloadKey]);
    ...
    <div className="bg-[#e5e7eb] min-h-screen text-gray-800 pb-16 font-sans">
      ...
      <section id="catalog" className="max-w-370 mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        ...
        <nav aria-label="Breadcrumb" className="mb-4 flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <span className="font-semibold text-[#007e42]">Thuốc BVTV</span>
        </nav>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 flex flex-col gap-5 min-w-0">
            ...
            <div className="bg-white rounded-xl border border-gray-100 px-5 py-3 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="flex items-center justify-between w-full sm:w-auto gap-4">
                ...
                <p className="text-xs font-bold text-gray-400">
                  ...
                  trong số <span className="text-gray-800">{total}</span> sản phẩm thuốc BVTV
                </p>
              </div>
            ...
            {/* Category not found */}
            {!loading && !error && categoryLoaded && !thuocCategory && (
              <div className="bg-white rounded-2xl p-16 border border-dashed border-gray-200 text-center flex flex-col items-center gap-3">
                <ILeaf size={40} />
                <h3 className="font-extrabold text-gray-800 text-base">Chưa có danh mục &ldquo;Thuốc BVTV&rdquo;</h3>
                <p className="text-xs text-gray-400 max-w-sm">
                  Vui lòng tạo danh mục với slug <code className="bg-gray-100 px-1 rounded">thuoc-bvtv</code> trong trang quản trị.
                </p>
              </div>
            )}

            {/* Empty */}
            {!loading && !error && thuocCategory && total === 0 && (
              <div className="bg-white rounded-2xl p-16 border border-dashed border-gray-200 text-center flex flex-col items-center gap-3">
                <ILeaf size={40} />
                <h3 className="font-extrabold text-gray-800 text-base">Không tìm thấy loại thuốc phù hợp</h3>
                <p className="text-xs text-gray-400 max-w-sm">Hãy thử thay đổi điều kiện lọc, nhóm thuốc, thương hiệu, khoảng giá hoặc từ khóa.</p>
                <button
                  onClick={handleClearFilters}
