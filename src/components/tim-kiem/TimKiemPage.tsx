...
export default function TimKiemPage({ initialQuery }: { initialQuery: string }) {
  /* ── Catalog state ── */
  ...
  /* ── Đồng bộ khi người dùng tìm từ navbar (đổi initialQuery) ── */
  useEffect(() => {
  ...
  }, [page, search, priceRange, reloadKey]);
  ...
  return (
    <div className="bg-[#e5e7eb] min-h-screen text-gray-800 pb-16 font-sans">
      {/* ── Catalog Section ── */}
      <section className="max-w-370 mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        {/* Breadcrumb */}
        <nav aria-label="Breadcrumb" className="mb-4 flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <span className="font-semibold text-[#007e42]">Tìm kiếm</span>
        </nav>
          ...
          <h1 className="text-xl font-extrabold text-gray-800 sm:text-2xl">
            {search.trim() ? (
              <>
                ...
                <span className="text-[#007e42]">&laquo;{search.trim()}&raquo;</span>
              </>
            ) : (
              "Tìm kiếm sản phẩm"
            )}
            ...
            <div className="fixed inset-0 z-50 bg-black/50 lg:hidden flex justify-end" onClick={() => setMobileFiltersOpen(false)}>
              <div className="bg-white w-80 h-full p-6 overflow-y-auto space-y-5" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between border-b pb-4">
                  <h3 className="text-sm font-black text-gray-800">Bộ Lọc</h3>
            ...
            {!loading && !error && total === 0 && (
              <div className="bg-white rounded-2xl p-16 border border-dashed border-gray-200 text-center flex flex-col items-center gap-3">
                ...
                <p className="text-xs text-gray-400 max-w-sm">
                  {search.trim()
                    ? "Hãy thử từ khóa khác, hoặc điều chỉnh khoảng giá."
                    : "Gõ tên sản phẩm vào ô tìm kiếm ở trên để bắt đầu."}
                </p>
                {search.trim() && (
                  <button
                    onClick={handleClearFilters}
